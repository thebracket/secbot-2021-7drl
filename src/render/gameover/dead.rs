...
pub fn game_over_dead(ctx: &mut BTerm, ecs: &World) -> NewState {
    ...
    let backdrop = XpFile::from_resource("resources/skull.xp").unwrap();
    ...
    batch.print_color(
        Point::new(50, 7),
        format!("SecBot survived for {} turns.", stats.turns_elapsed),
        bw,
    );
    batch.print_color(
        Point::new(50, 8),
        format!(
            "A total of {} things were killed/destroyed.",
            stats.total_dead
        ),
        bw,
    );
    batch.print_color(
        Point::new(50, 9),
        format!("{} props were smashed.", stats.total_props_smashed),
        bw,
    );
    batch.print_color(
        Point::new(50, 10),
        format!("{} monsters died.", stats.total_hostiles_killed),
        bw,
    );

    batch.print_color(
        Point::new(50, 11),
        format!("Caused ${} in property damage.", status.property_damage),
        bw,
    );
    ...
    batch.print_color(
        Point::new(50, 12),
        format!("Human Resources were: {}", phrase),
    ...
    batch.print_color(
        Point::new(50, 13),
        format!(
            "There were {} colonists in the game:",
            status.colony.total_colonists
        ),
        bw,
    );
    batch.print_color(
        Point::new(50, 14),
        format!("  ... of whom {} were rescued,", status.colony.rescued),
        bw,
    );
    batch.print_color(
        Point::new(50, 15),
    ...
    batch.print_color(
        Point::new(50, 16),
        format!(
            "  ... {} were already dead when you got there,",
            status.colony.located_dead
        ),
        bw,
    );

    batch.print_color(Point::new(50, 22), "The last thing SecBot heard:", bw);
    batch.print_color(
        Point::new(50, 23),
        stats.last_heard,
